...
export const address = document.getElementById("uv-address");
...
export function searchProxy() {
  ...
  if (localStorage.getItem("proxy") === "rammerhead") {
    address?.addEventListener('keydown', async function (event: KeyboardEvent) {
      ...
      if (event.key === 'Enter') {
        const encodedUrl: string = await RammerheadEncode(search((address as HTMLInputElement)?.value));
    ...
    if ('serviceWorker' in navigator) {
      ...
      navigator.serviceWorker.register(swFile, { scope: swConfigSettings.prefix })
        .then(async (registration) => {
          ...
          await setTransports();
          //w
          address?.addEventListener('keydown', async (event: KeyboardEvent) => {
            event.preventDefault();
            if (event.key === 'Enter') {
              await setTransports();

              const encodedUrl: string = swConfigSettings.prefix + __uv$config.encodeUrl(search((address as HTMLInputElement)?.value));
              sessionStorage.setItem("encodedUrl", encodedUrl);
              location.href = "/go";
            }
          });
